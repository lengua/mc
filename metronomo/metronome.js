...
var Metronome = {
	...
	// Testeo de accurancy
	tests: null, // Tiempo inicial
	teste: 0, // Cuantas vueltas esperar para contar
	testi: 60, // Cuantas vueltas contar
	...
	tick: function () {
		...
		if (Metronome.debug) {
			...
			// Testeo de Accurancy
			Metronome.testi--;
			console.log('tick:',Metronome.testi);
			if(Metronome.testi<=0) {
				// Salida de calculo
				console.log("Estop",Metronome.testi,performance.now()-Metronome.tests);
				window.clearInterval(Metronome.interval);
				Metronome.interval = null;
				Metronome.beat = 0;
			} else if (Metronome.testi==60) {
				// Inicio
				Metronome.tests = performance.now();
				Metronome.teste = 0;
				console.log("Estar",Metronome.testi,performance.now()-Metronome.tests);
			}
		}
